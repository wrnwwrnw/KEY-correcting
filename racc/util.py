...
class RACC_Cluster():
    ...
    def update_kv_decode(self, key_states, value_states, sheet, layer_idx, cache, count, kv_seq_len, cache_kwargs, ifindex, cacheidx):
        ...
        #修改压缩率后记得修改第100行，60对应144，50对应180，40对应216
        #实例化单层的注意力张量
        ...
        if non_topk_indices_expanded.shape[2]<=144 :
            ...
            cache.layers[layer_idx].keys = torch.cat([cache.layers[layer_idx].keys, k_drop], dim=2).to(torch.float32)
            cache.layers[layer_idx].values = torch.cat([cache.layers[layer_idx].values, v_drop], dim=2)
            if layer_idx == 31:
                ifindex[0] = 0
                #完成普通填充，可以构建
        #gather
        ...
        if len(cacheidx[layer_idx]) > 0:
            ...
            search_key = cache.layers[layer_idx].keys.to(key_states.device).gather(dim = 2, index = search_indices_expaned).to(key_states.dtype)
            search_value = cache.layers[layer_idx].values.to(key_states.device).gather(dim = 2, index = search_indices_expaned)
    ...
    def update_kv_decode_concise(self, key_states, value_states, sheet, layer_idx, count, kv_seq_len, cache_kwargs):
        #count是开始位置，index是相对位置
        head_dim = key_states.shape[3]
        top_k = int((sheet.shape[3] / 100) * 60)
        #修改压缩率后记得修改第100行，60对应144，50对应180，40对应216
...
def init_racckv(self):
    if not hasattr(self, "kv_cluster"):
        if not hasattr(self.config, 'window_size'):
            self.config.window_size = 128
        if not hasattr(self.config, 'max_capacity_prompt'):
            self.config.max_capacity_prompt = 6000
        if not hasattr(self.config, 'kernel_size'):
            self.config.kernel_size = 5
        if not hasattr(self.config, 'pooling'):
            self.config.pooling = 'avgpool'
    self.kv_cluster = RACC_Cluster( 

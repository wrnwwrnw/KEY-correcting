    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    if (
        ...
        if os.environ.get("RESTART") == "1":
            ...
            del mistral_attention_forward.cache
        ...
        if not hasattr(mistral_attention_forward, "cacheidx"):
            mistral_attention_forward.cacheidx = [[] for _ in range(32)]
        if not hasattr(mistral_attention_forward, "index_grid"):
            mistral_attention_forward.index_grid = [[None for _ in range(32)] for _ in range(32)]
        if not hasattr(mistral_attention_forward, "ifindex"):
        #    manager = multiprocessing.Manager()
        #    mistral_attention_forward.ifindex = manager.list([-1])
            mistral_attention_forward.ifindex = [-1]
        if mistral_attention_forward.ifindex[0]==0 :
            #    executor = ProcessPoolExecutor()
            mistral_attention_forward.ifindex[0] = 1 
            #    pool = Pool(10)
            #    result = pool.apply_async(build, (mistral_attention_forward.cache, mistral_attention_forward.index_grid, mistral_attention_forward.ifindex))
            #    pool.close()
            #    future = executor.submit(build ,mistral_attention_forward.cache ,mistral_attention_forward.index_grid ,mistral_attention_forward.ifindex)
            #    executor.shutdown(wait=False)
            build(mistral_attention_forward.cache ,mistral_attention_forward.index_grid ,mistral_attention_forward.ifindex)
        ...
        if past_key_values is not None:
            ...
            #预填充处理
                ...
                if self.layer_idx ==0:
                    mistral_attention_forward.count1 = kv_seq_len
                    mistral_attention_forward.count2 = 0
                    mistral_attention_forward.conut3 = key_states_compress.shape[2]
            else:
                if self.layer_idx ==0:     
                    mistral_attention_forward.count2 += 1
                self.kv_seq_len += q_len
                key_states, value_states = past_key_values.update(key_states, value_states, self.layer_idx, cache_kwargs)
                # 核心代码
                if (mistral_attention_forward.count2%200==0)and(mistral_attention_forward.count2%400!=0)and(mistral_attention_forward.ifindex[0]==2):
                ...
                if mistral_attention_forward.count2>360:
                    ...
                    if not hasattr(mistral_attention_forward, "sheet"):
                        mistral_attention_forward.sheet = torch.zeros(32, 40, 32, key_states.shape[2]-mistral_attention_forward.conut3-1)
                    head_dim = 128
